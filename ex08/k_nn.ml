...
let rec restruct_l l_radar radar closer l_closer =
    match l_radar with
        |[] -> l_closer
        |head::tail -> if (eu_dist (get_1_2 radar) (get_1_2 head)) 
                            < (eu_dist (get_1_2 radar) (get_1_2 closer))
                then restruct_l tail radar closer l_closer @ [head]
                else restruct_l tail radar head l_closer @ [closer]

let count_str l_radar =
    let rec rec_count l_radar str count = 
        match l_radar with
            |[] -> count
            |head::tail -> if get_2_2(head) = str then rec_count tail str (count + 1) else rec_count tail str count
    in
    let thestr = get_2_2 (List.nth l_radar 0) in
    if (rec_count l_radar thestr 0) > 2
        then thestr
    else thestr
        


let k_nn l_radar nb radar =
    match l_radar with
    |[] -> ""
    |head::tail -> (
        let rec rec_k_nn l_radar closer =
            match l_radar with
                |[] -> closer
                |hydra::rest -> rec_k_nn rest (restruct_l closer radar hydra [])
        in
        get_2_2(List.nth (rec_k_nn tail [head]) 0)
    )
...
let () =
    ...
    match l_radar with
    |[] -> ()
    |head::tail -> print_string "prediction ";print_endline(k_nn l_radar 3 head);
    print_string "reality ";print_endline (get_2_2 head);
    print_string "prediction ";print_endline(k_nn l_radar2 2 head);
    print_string "reality ";print_endline (get_2_2 head)
